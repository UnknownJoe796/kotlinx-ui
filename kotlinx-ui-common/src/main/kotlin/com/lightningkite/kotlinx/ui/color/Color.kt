...
import com.lightningkite.kotlinx.bytes.toByteArray
import com.lightningkite.kotlinx.bytes.toStringHex
...
data class Color(
    ...
    companion object {
        ...
        fun hsvInterpolate(left: Color, right: Color, ratio: Float): Color = HSVColor.interpolate(left.toHSV(), right.toHSV(), ratio).toRGB()
    }
    ...
    fun toHSV(): HSVColor = HSVColor(
            ...
            hue = when {
                ...
                (green > red && green > blue) -> (blue - red).div(max(max(red, green), blue) - min(min(red, green), blue)).plus(2)
                (blue > green && blue > red) -> (red - green).div(max(max(red, green), blue) - min(min(red, green), blue)).plus(4)
                else -> 0f
            }.plus(6f).rem(6f).div(6f),
            saturation = run {
    ...
    fun toWeb(): String {
        return "#" + this.toInt().toByteArray().toStringHex()
    }

    fun toAlphalessWeb(): String {
        return "#" + this.toInt().toByteArray().toStringHex().drop(2)
    }
}
